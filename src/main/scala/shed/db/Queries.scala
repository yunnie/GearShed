...
object Queries {
  ...
  /* For an example of droping tables and creating tables
   * using Doobie see: 
   *   https://tpolecat.github.io/doobie/docs/07-Updating.html
   * specifically look at the section under Data Defintion.
   */
  ...
  // TO DO Create Indexs for the tables to speed joins  
  
  // Queries
  // Get a list of items
  // Note that sql"""[statement]""" returns Fragment type
  ...
  // Remove an item from the catelog
  ...
  def removeItemTags(itemId: Int): Update0 = 
    /* When removing an item form the Item table
     * it helps to also remove the tags assocaited
     * with the item from the Item2Tag table
     */
    sql"""
  ...
  def insertTagList: Update[TagLabel] = {
    ...
    Update[TagLabel](query) // .updateMany(tagList) returns ConnectionIO[Int]
    /* For refernce see the Update trait, and the Update companion object.
     * and the section in the book of doobie on batch updates
     * basically, once you hace an Update[Tag] item, you
     * use either .updateMany(tagList) or updateManyWithGeneratedKeys[Tag]
     * ("id", "label")(tagList), assuming that the Tag class had
     * a slot for the id
     */
  }
